...
from pnmf.serdp09_reference import (
    Serdp09ReferenceError,
    Serdp09TrustedProof,
    evaluate_case,
    load_case,
)
...
def _write_archive(
    ...
    *,
    ...
    preamble: tuple[str, ...] = (),
    workbook_rows: list[tuple[object, ...]] | None = None,
    workbook_header: tuple[str, ...] = ("icrdg", "config"),
) -> None:
...
def test_load_case_accepts_declared_auxiliary_header_before_numeric_rows(tmp_path: Path):
    _write_archive(
        tmp_path,
        preamble=(
            'TITLE="SERDP09"',
            'DATASETAUXDATA scenario="Lossless Conditions"',
        ),
        header='VARIABLES="Hz" "Polar angle" "PSD (dB)"',
        zone='ZONE T="844" I=601 J=2 F=POINT\nAUXDATA rdg="844"',
    )
...
def test_evaluate_case_reports_both_unproven_compatibility_requirements(
    tmp_path: Path,
):
    _write_archive(
        tmp_path,
        workbook_header=("icrdg", "stream_mapping_proof", "frequency_basis_proof"),
        workbook_rows=[(844, "proven", "proven")],
    )

    result = evaluate_case(load_case(tmp_path, 844))

    assert result.verdict == "incompatible"
    assert result.shape_rmse_db_by_angle == ()
    assert result.broadband_directivity_rmse_db is None
...
def test_evaluate_case_compares_only_normalized_synthetic_metrics(tmp_path: Path):
    _write_archive(
        tmp_path,
        workbook_header=(
            "icrdg",
            "stream_mapping_proof",
            "frequency_basis_proof",
        ),
        workbook_rows=[(844, "proven", "proven")],
    )
    ...
    result = evaluate_case(
        load_case(tmp_path, 844),
        trusted_proof=Serdp09TrustedProof(True, True),
    )

    assert result.verdict == "compatible"
    assert result.reasons == ()
    assert result.reference_angle_deg == 90.0
    assert result.normalization_order == (
        "per-angle spectral peak normalization",
        "broadband energetic sum",
        "single 90.0 degree angular normalization",
    )
    assert len(result.shape_rmse_db_by_angle) == 2
    assert all(np.isfinite(metric.rmse_db) for metric in result.shape_rmse_db_by_angle)
    assert result.broadband_directivity_rmse_db is not None
    assert np.isfinite(result.broadband_directivity_rmse_db)
    assert all(np.isfinite(level.observed_db) and np.isfinite(level.predicted_db)
               for level in result.broadband_levels_by_angle)
    reference = next(level for level in result.broadband_levels_by_angle
                     if level.angle_deg == result.reference_angle_deg)
    assert reference.observed_db == 0.0
    assert reference.predicted_db == 0.0


def test_evaluate_case_never_uses_calibration_or_mixed_jet_fallback(
    ...
    from pnmf.physics import JetSource, PhysicsNPDModel

    def forbidden(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("forbidden evaluator path")

    monkeypatch.setattr(PhysicsNPDModel, "calibrate", forbidden)
    monkeypatch.setattr(JetSource, "spectrum_with_diagnostics", forbidden)
